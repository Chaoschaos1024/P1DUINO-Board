{{
This demo outputs all of the data available for all four blobs using mode 1.
...
PUB start  |i,tempcnt ,xpos ,mody,blob
  ...
  repeat
     ...
     repeat blob from 0 to 3
        text.out($0A)
        text.out((blob*8)+6)
        text.dec(wii.getx(@out_data,1,blob))
     text.out($0D)
     ...
     repeat blob from 0 to 3
        text.out($0A)
        text.out((blob*8)+6)
        text.dec(wii.gety(@out_data,1,blob))
     text.out($0D)
