...
{-----------------REVISION HISTORY-----------------
 ...
 as TV_TerminalHoming, Eric Ratliff 6/7/2008 added "home" function to return writing point to upper left without
                       clearing screen, to eliminate flicker in programs that often repaint characters
                       Eric Ratliff 8/10/2009 adding 'line' function, similar to home function
                       }
...
CON
  ...
  x_margin = x_spacing + 1 ' enough to over write         
...
VAR
  ...
  long  x_left_pix, y_upper_pix, x_right_pix, y_lower_pix        ' corners of character being written
...
''  $20..$7E = character
  ...
  case c
    ...
    $20..$7E:           'character?
      x_left_pix := x * x_chr
      x_right_pix := x_left_pix + x_spacing + 1
      y_lower_pix := -y * y_chr
      y_upper_pix := y_lower_pix - y_offset
      gr.color(0) ' make drawing color black
      gr.quad(x_left_pix,y_upper_pix,x_left_pix,y_lower_pix,x_right_pix,y_lower_pix,x_right_pix,y_upper_pix)
      gr.color(1) ' make drawing color white
      gr.text(x_left_pix, y_upper_pix, @c)
      gr.finish
      if ++x == x_limit
        newline
...
PUB line(LineIndex)
'' move writing place to left of specified line without clearing screen
  if LineIndex => 0 and LineIndex =< y_max
    x := 0
    y := LineIndex
