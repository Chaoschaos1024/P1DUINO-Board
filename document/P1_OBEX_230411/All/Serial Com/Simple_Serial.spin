...
PUB start(rxPin, txPin, baud)
  ...
  if started
    ...
    bitTime := (clkfreq / ||baud)                         ' calculate serial bit time added a *2 to compensate for 10 MHz crystal 
...
PUB print(data)
  str(number.ToStr(data, number#DEC))   
PUB cls                                                 'the hyperterminal clearscreen function
  tx($c)

PUB drop | dropit                                       'the hyperterminal equivalent of pressing enter twice
  tx($d)
  tx($a)
  tx($a)
